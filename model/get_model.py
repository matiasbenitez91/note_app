...
def get_entropy(prob):
	p=np.array(prob)
	entropy=(-p * np.log2(p)-(1-p)*np.log2(1-p)).sum()
...
#DEFINE THE MODEL
def get_model(train, train_lab,val, val_lab,test, test_lab, opt, entropy_data):
					...
					for _, train_index in kf:
						...
						if uidx % opt.valid_freq == 0:
							...
							if val_auc_mean > max_val_auc_mean:
								...
								entropy_prob, entropy_logits = sess.run([prob_, logits_],
																  feed_dict={x_: x_entropy_batch, x_mask_: x_entropy_batch_mask, 
																			 keep_prob: 1.0,class_penalty_:0.0})
							else:
								pass	
	...
	return test_auc_mean, max_val_auc_mean, [validation_list, test_list], get_entropy(entropy_prob)
def main():
	#change this values
	...
	opt.emb_size=300
	opt.H_dis=150
	opt.max_epochs=600#opt.restore=True
	train=noteix_0
	val=noteix_test[:100]
	train_lab=y[:,2]
	...
	entropy_data=database.loc[database['valid'],'tokens_num']

	_, max_auc, list_aucs, entropy=get_model(train, train_lab, val, val_lab, test, test_lab, opt, entropy_data)
	with open('model/kidney/initial_auc.p','wb') as f:
		pickle.dump(list_aucs, f)
	df=pd.DataFrame({'AUC_test': [_],'AUC_val':[max_auc]})
	database.loc[:,'entropy']=float('-inf')
	database.loc[database['valid'],'entropy']=entropy
	database.to_csv('database.csv', index=False)
	df.to_csv('model/kidney/max_auc.csv', index=False)
	k=pd.DataFrame({'Report#':['initial'],'AUC_test':[_],'AUC_val':[max_auc], 'time':[strftime("%Y-%m-%d %H:%M:%S", gmtime())]})
