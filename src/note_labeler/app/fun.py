...
import re
artifacts_path="data/"
def length_db(artifacts_path):
	db=pd.read_csv(os.path.join(artifacts_path,'database.csv'), encoding="latin-1")
	return len(db)

def data_to_dict(num):
	le=pd.read_csv(os.path.join(artifacts_path,"model_iteration",'auc.csv'))
	db=pd.read_csv(os.path.join(artifacts_path,'database.csv'), encoding="latin-1")
	#db=data.sample(1)
...
def change_database_invalid(num, row):
	...
	db.to_csv(os.path.join(artifacts_path,'database.csv'), index=False, encoding="latin-1")
...
def change_database_existing(num, row):
	...
	db.loc[num, 'valid']=False
	db.loc[num, 'determinants']=float('inf')
	db.to_csv(os.path.join(artifacts_path,'database.csv'), index=False)
...
def add_report_train(num, dict):
	...
	file=pickle.load(open(os.path.join(artifacts_path,"model_iteration",'train.pkl'), 'rb'))
	file[0].append(token)
	file[2]=np.append(file[2], np.array([labels]), axis=0)
	file[-1]=max(file[-1], len(token))
	pickle.dump(file,open(os.path.join(artifacts_path,'model_iteration','train.pkl'), 'wb'))
